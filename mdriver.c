    ...
    while ((c = getopt(argc, argv, "f:t:hvVgal")) != EOF) {
        switch (c) {
            ...
            case 't': /* Directory where the traces are located */
                ...
                break;
            case 'a': /* Don't check team structure */ team_check = 0; break;
            case 'l': /* Run libc malloc */ run_libc = 1; break;
            case 'v': /* Print per-trace performance breakdown */
                verbose = 1;
                break;
            case 'V': /* Be more verbose than -v */ verbose = 2; break;
            case 'h': /* Print this message */ usage(); exit(0);
